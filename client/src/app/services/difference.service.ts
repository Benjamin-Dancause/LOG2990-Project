...
export class DifferenceService {
    ...
    findDifference(ctx1: CanvasRenderingContext2D, ctx2: CanvasRenderingContext2D, radius: number): HTMLCanvasElement {
        const pixels1 = ctx1.getImageData(0, 0, 640, 480) as ImageData;
        const pixels2 = ctx2.getImageData(0, 0, 640, 480) as ImageData;
        const diff = ctx1.createImageData(640, 480) as ImageData;

        for (let i = 0; i < (pixels1.data.length as number); i += 4) {
            if (
    ...
    countDifference(canvas: HTMLCanvasElement): number {
        ...
        for (let i = 0; i < (pixels3.data.length as number); i += 4) {
